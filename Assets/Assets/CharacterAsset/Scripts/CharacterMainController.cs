...
namespace StarterAssets
{
    ...
    public class CharacterMainController : MonoBehaviour
    {
        ...
        //Instance (this is for easy referencing of the one  Character in the scene, Instance is set up in OnValidate())
        [HideInInspector] public static CharacterMainController Instance;
        ...
        [HideInInspector] public GameplayType appliedGameplayType = CharacterMainController.GameplayType.FirstPerson;
        [HideInInspector] public Model appliedCharacterModel = CharacterMainController.Model.Capsule;
        //This is space for the "Apply Gameplay Changes" button generated in UnityEditorForCharacterMainController.cs
        [Space(40)]
        ...
        private void applyGameplayChanges()
        {
            ...
            FollowCam = GameObject.Find("StarterAssetsFollowCamera");
            DestroyImmediate(FollowCam.GetComponent<CinemachineVirtualCamera>().GetCinemachineComponent<Cinemachine3rdPersonFollow>());
            //Create an editable Cinemachine Virtual Camera
            FollowCam.GetComponent<CinemachineVirtualCamera>().AddCinemachineComponent<Cinemachine3rdPersonFollow>();
            Cinemachine3rdPersonFollow newCamComponent = FollowCam.GetComponent<CinemachineVirtualCamera>().GetCinemachineComponent<Cinemachine3rdPersonFollow>();
            ...
            {
                case CharacterMainController.GameplayType.FirstPerson:
                ...
                case CharacterMainController.GameplayType.ThirdPerson:
            ...
            switch (appliedCharacterModel)
            {
                ...
                case CharacterMainController.Model.DefaultArmature:
                    model = sResourceGrabbingFunctions.addPrefabFromAssetsByName("DefaultArmatureCharacterModel", modelParent, false, Vector3.zero);
                    model.transform.localPosition = sResourceGrabbingFunctions.getPrefabFromResourcesByName("DefaultArmatureCharacterModel").transform.localPosition;
                    break;
                case CharacterMainController.Model.Capsule:
                    model = sResourceGrabbingFunctions.addPrefabFromAssetsByName("CapsuleCharacterModel", modelParent, false, Vector3.zero);
                    model.transform.localPosition = sResourceGrabbingFunctions.getPrefabFromResourcesByName("CapsuleCharacterModel").transform.localPosition;
                    break;
                case CharacterMainController.Model.Voxel:
                    model = sResourceGrabbingFunctions.addPrefabFromAssetsByName("VoxelCharacterModel", modelParent, false, Vector3.zero);
                    model.transform.localPosition = sResourceGrabbingFunctions.getPrefabFromResourcesByName("VoxelCharacterModel").transform.localPosition;
                    break;
                case CharacterMainController.Model.None:
                    break;
            }
        }
        ...
        private void setUpCinemachineScreenShakeNoiseProfile()
        {
            ...
            FollowCam = GameObject.Find("StarterAssetsFollowCamera");
            FollowCam.GetComponent<CinemachineVirtualCamera>().GetCinemachineComponent<CinemachineBasicMultiChannelPerlin>().m_NoiseProfile = shake;
        }
        ...
        private void Start()
        {
            ...
            _hasAnimator = sResourceGrabbingFunctions.TryGetComponentInChildren<Animator>(modelParent);
        ...
        private void Update()
        {
            ...
            addConstantForceToCharacter(new Vector3(0.0f, _verticalVelocity, 0.0f));
        ...
        //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        public void addConstantForceToCharacter(Vector3 addedForce)
        {
            int emptyIndex = -1;
            for (int i = 0; i < appliedForces.Length; i++)
            {
                ...
                if (i == appliedForces.Length - 1)
                {
                    Debug.Log("Cannot add any more forces to character controller in this singular frame. To add more forces to the character" +
                        " please increase APPLIED_FORCES_AMOUNT in CharacterMainController.cs .");
                    return;
                }
        ...
        public void addImpulseForceToCharacter(Vector3 direction, float force)
        {
            direction.Normalize();
