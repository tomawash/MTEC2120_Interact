...
public class ABILITY_CharacterMovement : AbilityBaseClass
{
    [Header("Basics")]
    ...
    private void Reset()
    {
        if (GetComponentInParent<CharacterMainController>().appliedGameplayType == CharacterMainController.GameplayType.FirstPerson){
            rotateTowardMoveDirection = false; }
        if (GetComponentInParent<CharacterMainController>().appliedGameplayType == CharacterMainController.GameplayType.ThirdPerson) {
    ...
    public override void abilityStart()
    {
        ...
        _hasAnimator = sResourceGrabbingFunctions.TryGetComponentInChildren<Animator>(CharacterMainController.Instance.modelParent);
        if (_hasAnimator) { _animator = CharacterMainController.Instance.modelParent.GetComponentInChildren<Animator>(); }
    ...
    public override void abilityUpdate() 
    {
        ...
        if (passedMovementInput != Vector2.zero)
        {
            ...
            // move the player
            CharacterMainController.Instance.addConstantForceToCharacter(targetDirection.normalized * _speed);
