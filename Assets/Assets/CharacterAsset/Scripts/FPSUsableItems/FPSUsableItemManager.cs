...
public class FPSUsableItemManager : MonoBehaviour
{
    ...
    private void updateUsableItemsArray()
    {
        ...
        FPSUsableItemBaseClass[] grabbedUsableItems;
        ...
        if (GetComponent<FPSUsableItemBaseClass>() != null)
        {
            grabbedUsableItems = GetComponents<FPSUsableItemBaseClass>();
        }
        else
        {
            grabbedUsableItems = new FPSUsableItemBaseClass[0];
        }

        foreach (FPSUsableItemBaseClass item in grabbedUsableItems)
        {
    ...
    {
        public FPSUsableItemBaseClass script;
        [HideInInspector] public GameObject prefab;

        public usableItemScriptAndPrefab(FPSUsableItemBaseClass passedScript)
        {
            script = passedScript;
            prefab = sResourceGrabbingFunctions.getPrefabFromResourcesByName(script.getNameOfPrefabToLoad());
        }
    }
    ...
    public void Reset()
    {
        ...
        GameObject FPSUsableItemsCamera = sResourceGrabbingFunctions.addPrefabFromAssetsByName("FPSUsableItemsCamera", GameObject.Find("PlayerCameraRoot").transform, false, Vector3.zero);
