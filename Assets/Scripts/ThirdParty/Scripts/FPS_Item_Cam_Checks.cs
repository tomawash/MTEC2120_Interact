...
public class FPS_Item_Cam_Checks : MonoBehaviour
{
    ...
    public void Reset()
    {
        int layerCheck = LayerMask.NameToLayer("FPSUsableItem");
        if (layerCheck == -1)
        {
            Debug.LogError("New layer FPSUsableItem must be created and assigned in Layer Manager so FPSUsableItems can be seen by the FPSUsableItemsCamera and Player.");
        }
        ...
        if (renderPipelineAsset != null)
        {
            ...
            {
                Debug.LogWarning("Some additional code has to run for the  FPS Item camera to work with the Universal Render Pipeline." +//Delete this line if URP is Used
                    "Open the FPS_Item_Cam_Checks.cs Script and uncomment lines 32-44. Then add the  Character Again."); //Delete this line if URP is Used
